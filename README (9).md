...
export function renderOptionTiles(stepId) {
  ...
  // Pass 3: profile options are constrained by the selected family.
  if (stepId === 'profile' && state.selections.family) {
    options = getCompatibleProfiles(state.selections.family);
  }
  // Pass 5: leg + shelf options are also constrained by the selected family.
  if ((stepId === 'leg' || stepId === 'shelf') && state.selections.family) {
  ...
  el.browserGrid.innerHTML = options.map((opt) => {
    ...
    return `<li>
      <button
        ...
        <span class="laval-tile-swatch" data-swatch="${opt.swatch}" aria-hidden="true"></span>
...
export function selectOption(stepId, optionId, sourceEl = null) {
  ...
  if (stepId === 'family') {
    for (const downstream of ['profile', 'leg', 'shelf']) {
...
const PROFILE_FAMILY_MAP = {
  'tc-p1':'tailored-classical', 'tc-p2':'tailored-classical', 'tc-p3':'tailored-classical',
  'ss-p1':'soft-sculptural',    'ss-p2':'soft-sculptural',    'ss-p3':'soft-sculptural',
  'fo-p1':'french-ornate',      'fo-p2':'french-ornate',      'fo-p3':'french-ornate'
};

export function getCompatibleProfiles(familyId) {
  if (!familyId) return STEP_OPTIONS.profile;
  return STEP_OPTIONS.profile.filter((p) => PROFILE_FAMILY_MAP[p.id] === familyId);
}

// Pass 5: generic family-compatibility filter for leg + shelf (and any other
// step whose options carry a `compatibleFamilies` array). If the option has
// no compatibleFamilies field, treat it as universal (compatible with all).
...
// Pass 5: returns true if a recorded selection for stepId is still valid
// under the active family. Used to decide whether a stale selection must
// be cleared when the family changes.
...
export function renderMantelPreview() {
  ...
  const sel = state.selections;
  figure.dataset.family  = sel.family  || 'tailored-classical';
  figure.dataset.profile = sel.profile || '';
  figure.dataset.leg     = sel.leg     || '';
  figure.dataset.shelf   = sel.shelf   || '';
  figure.dataset.finish  = sel.finish  || '';
  mount.innerHTML = buildMantelSvg();
}
...
function buildEditorMarkup() {
  return `<div class="laval-nested-editor">
    ...
    <p class="laval-nested-editor-note">
      Material visuals are placeholder-stage only.
      Real stone library arrives in the production build.
    </p>
  </div>`;
}
...
export function renderFinishChips() {
  ...
  if (!body) return;
  const options   = STEP_OPTIONS.finish || [];
    ...
    ${options.map((opt) => {
      ...
      return `<li>
        <button
          ...
          aria-pressed="${isSel}">
          <span class="laval-finish-chip-swatch" data-swatch="${opt.swatch}" aria-hidden="true"></span>
  ...
  if (tabId === 'stone-finish') {
    ...
    body.innerHTML = `<div class="laval-material-placeholder">
      ...
      <span class="laval-material-placeholder-sub">
        Real material look library arrives in the production build.
        No asset generation or pricing in this stage.
      </span>
    </div>`;
  }
}
